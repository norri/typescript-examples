...
describe('PostgresDatabase', () => {
  ...
  test('insert should add a book to the database', async () => {
    ...
    expect(pool.query).toHaveBeenCalledWith('INSERT INTO items (title) VALUES ($1)', [newBook.title]);
  });
  ...
  test('getOne should return the correct book by ID', async () => {
    ...
    expect(pool.query).toHaveBeenCalledWith('SELECT id, title FROM items WHERE id = $1', [1]);
  });
  ...
  test('getOne should return null if the book does not exist', async () => {
    ...
    expect(pool.query).toHaveBeenCalledWith('SELECT id, title FROM items WHERE id = $1', [999]);
  });
});
